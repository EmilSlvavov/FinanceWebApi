...
/**
 * Page for budget reporting and analytics
 */
public class BudgetReportPage {
    public static void show(BudgetService budgetService) {
        ...
        while (true) {
            ...
            for (BudgetResponse budget : budgets) {
                Double spent = budget.getTotalSpent() != null ? budget.getTotalSpent() : 0;
                Double remaining = budget.getValue() - spent;
                Double percentUsed = (spent / budget.getValue()) * 100;
    ...
    private static void displayBudgetDetails(BudgetService budgetService, Integer budgetId) {
        ...
        System.out.println("Total Budget: " + budget.getValue());
