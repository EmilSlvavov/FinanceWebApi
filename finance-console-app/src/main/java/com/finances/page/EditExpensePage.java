...
/**
 * Page to edit an existing expense
 */
public class EditExpensePage {
    ...
    public static void show(ExpenseService expenseService, Integer expenseId) {
        ...
        if (expense == null) {
            ConsoleUI.printError("Expense not found.");
            ConsoleUI.pause();
            return;
        }
        ...
        switch (choice) {
            case "1":
                ...
                try {
                    ...
                    ExpenseResponse updated = expenseService.updateExpense(expenseId, expense.getExpenseCategoryId(), 
                        newAmount, expense.getExpenseDate(), expense.getIsRecurring(), expense.getDescription());
                    if (updated != null) {
                        ConsoleUI.printSuccess("Expense updated successfully!");
                    } else {
                        ConsoleUI.printError("Failed to update expense.");
                    }
            ...
            case "2":
                ...
                try {
                    ...
                    ExpenseResponse updated = expenseService.updateExpense(expenseId, expense.getExpenseCategoryId(), 
                        expense.getAmount(), newDate, expense.getIsRecurring(), expense.getDescription());
                    if (updated != null) {
                        ConsoleUI.printSuccess("Expense updated successfully!");
                    } else {
                        ConsoleUI.printError("Failed to update expense.");
                ...
                break;
            case "3":
                ExpenseResponse toggled = expenseService.updateExpense(expenseId, expense.getExpenseCategoryId(), 
                    expense.getAmount(), expense.getExpenseDate(), !expense.getIsRecurring(), expense.getDescription());
                if (toggled != null) {
                    ConsoleUI.printSuccess("Expense updated successfully!");
                } else {
                    ConsoleUI.printError("Failed to update expense.");
            ...
            case "4":
                ...
                ExpenseResponse updated = expenseService.updateExpense(expenseId, expense.getExpenseCategoryId(), 
                    expense.getAmount(), expense.getExpenseDate(), expense.getIsRecurring(), newDesc);
                if (updated != null) {
                    ConsoleUI.printSuccess("Expense updated successfully!");
                } else {
                    ConsoleUI.printError("Failed to update expense.");
